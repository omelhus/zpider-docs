...
- Unit convertion can now be enabled (using QuantityPerUnitSale for Visma Global)
...
## Fixed issues
 ...
 - Correcly dispose sql connections
 ...
 - Values on customer, orders etc can now be set to 0 if needed.
 - DefaultUriProvider now handles only having the gid passed to it. This means that <z:url gid=[groupnumber]/> now works correclty without adding a plugin.
 - The timeout in forgottenpassword is increased from 3 to 30 seconds.
 - Cultures in emails are now validated so that only activated cultures are used for new user during import.
